...
DWORD CNamedPipeClient::Connect(LPCTSTR lpszPipeName, PPIPE_CLIENT_ON_MESSAGE pOnMessage,
	PPIPE_CLIENT_ON_CONNECT pOnConnect,
	PPIPE_CLIENT_ON_DISCONNECT pOnDisconnect)
{
	DWORD dwErr = 0;
	...
	while (1)
	{
		...
		{
			DBG_ERROR("Could not open pipe. Error=%d\n", dwErr);
			return dwErr;
		}
		...
		{
			DBG_ERROR("Could not open pipe: 20 second wait timed out.");
			return -1;
		}
	}
	...
	{
		dwErr = GetLastError();
		DBG_ERROR("SetNamedPipeHandleState failed. GLE=%d\n", dwErr);
		...
		while (1)
		{
			...
			if (dwRes == WAIT_OBJECT_0 + 1)
			{
				if (m_bPendingIO)
				{
					...
					{
						//error happened
						DBG_ERROR("GetOverlappedResult failed 1. GLE=%d\n", GetLastError());
						break;
					}
...
DWORD CNamedPipeClient::SendData(LPVOID msg, DWORD msg_size)
{
	//lock
	...
	{
		//write completed
		DBG_INFO("Write successfully completed, dwWritten: %d\n", dwWritten);
	}
	else
	{
		...
		{
			DBG_INFO("WriteFile to pipe pending \n");

			BOOL bWaitResult = GetOverlappedResult(m_hPipe, &ovWrite, &dwWritten, TRUE);
			if (bWaitResult)
			{
				//write completed
				DBG_INFO("Write overlapped successfully completed, dwWritten: %d\n", dwWritten);
			}
			else
			{
				//error
				DBG_ERROR("WriteFile to pipe failed. GLE=%d\n", GetLastError());
				Disconnect();
		...
		{
			//error
			DBG_ERROR("WriteFile to pipe failed 2. GLE=%d\n", GetLastError());
			Disconnect();
...
DWORD CNamedPipeClient::ReadPipe()
{
	...
	else
	{
		...
		{
			//pending IO
			DBG_INFO("ReadFile from pipe pending \n");
			m_bPendingIO = TRUE;
		}
		else
		{
			//error
			DBG_ERROR("ReadFile from pipe failed. GLE=%d\n", dwErr);
			m_bPendingIO = FALSE;
			return dwErr;
		}
	}
