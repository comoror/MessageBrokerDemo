...
DWORD CNamedPipeServer::Run()
{
	...
	while (1)
	{
		...
		DBG_INFO("WaitForMultipleObjects %d\n", i);
	...
	if (m_instPipes[pipeIndex].mPendingIO) 
	{
		...
		case PipeStates::CONNECTING:
			if (!success) 
			{
				...
				DBG_ERROR("GetOverlappedResult fail: %d\n", dwErr);
...
DWORD CNamedPipeServer::ReadPipe(DWORD pipeIndex)
{
	...
	{
		DBG_INFO("Read successfully completed: pipe %d, BytesRead: %d\n", 
			pipeIndex, m_instPipes[pipeIndex].mBytesRead);
	...
	{
		DBG_INFO("ReadFile to pipe pending: pipe %d\n", pipeIndex);
...
DWORD CNamedPipeServer::WritePipe(DWORD pipeIndex, LPVOID msg, size_t msg_size)
{
	...
	DBG_INFO("WriteFile to pipe %d, bytesWritten: %d.\n", pipeIndex, bytesWritten);

	expectedBytesWritten =	static_cast<DWORD>(msg_size);

	if (success && bytesWritten == expectedBytesWritten) 
	{
		DBG_INFO("Write successfully completed: pipe %d, bytesWritten: %d\n", pipeIndex, bytesWritten);
		return 0;
	}
...
VOID CNamedPipeServer::BroadcastData(LPVOID msg, size_t msg_size)
{
	for (int i = 0; i < nMaxPipes; i++)
	{
		if (m_instPipes[i].mCurrentState != PipeStates::CONNECTING)
		{
			SendData(i, msg, msg_size);
		}
	}
}
